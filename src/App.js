    ...
    async componentDidMount() {
        ...
        flightPurchased.watch(function(err, result) { 
            ...
            if(customer === this.state.account) {
                this.container.warning(`You purchased a flight to ${flight} with a cost of ${price}`,
                 'Flight Purchased');
            } else {
                console.log(`One customer purchased a flight to ${flight} 
            ...
            <div className="row">
                ...
                <div className="col-sm">
                    <Panel title="Loyalty points - refundable ether">
                        ...
                        <button className="btn btn-sm btn-success" onClick={this.refundLoyaltyPoints.bind(this)}>Redimir Saldo</button>
                    </Panel>
                </div>
            </div>
            <div className="row">
                <div className="col-sm">
                    <Panel title="Available flights">
                        {this.state.flights.map((flight,i) => {
                            return <div key={i}>
                                <span> {flight.name} - costo: {this.toEther(flight.price)} ETH</span>
                                <button className="btn btn-sm btn-primary" onClick={() => this.buyFlight(i, flight)}> Comprar Boleto</button>
                             </div>
                        })}
                    </Panel>
                </div>
                <div className="col-sm">
                    <Panel title="Your flights">
                        {this.state.customerFlights.map((flight, i) => {
                            return <div key={i}>
                            Vuelo: {flight.name} - Costo: {this.toEther(flight.price)} ETH
                            </div>
                        })}
                    </Panel>
                </div>
