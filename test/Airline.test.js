...
contract('Airline', accounts => {
    ...
    it('should allow customers to buy a flying providing its value', async() => {
    ...
    it('should allow customers to redeem loyalty points', async() => {
        let flight = await instance.flights(2);
        let price = flight[1];

        await instance.buyFlight(1, { from: accounts[0], value: price});

        let balance = await web3.eth.getBalance(accounts[0]); //consultando el balance de la cuenta
        await instance.redeemLoyaltyPoints({ from: accounts[0]}); //cambiando los loyaltyPoints por saldo en ether usando la funcion
        let finalBalance = await web3.eth.getBalance(accounts[0]); //almacenando el balance de la cuenta luego de haber redimido los LP por saldo

        let customer = await instance.customers(accounts[0]);
        let loyaltyPoints = customer[0];

        assert(loyaltyPoints, 0); // comprobando que los LP se devuelven a 0 una vez que se han canjeado
        assert(finalBalance > balance); //verificando que el balanceFinal(luego de cangear los LP) es mayor que el inicial 
    });
});